...
window.onload = function () {
    ...
    function documentActions(e) {
        ...
        if (targetElement.classList.contains('profile-ico-active-use')) {
            targetElement.closest('.profile-active-btn').nextElementSibling.classList.toggle('_clicked-button-prf');
        }
    }
}
...
const formContents = document.querySelectorAll(".form-booking__content");

let formContentsNum = 0;
...
function updateFormContents() {
        ...
        formContents.forEach(formContent => {
            formContent.classList.contains("_active") &&
            formContent.classList.remove("_active");
        });

    formContents[formContentsNum].classList.add("_active");
}
...
function checkedFunc() {
    var check = document.getElementsByClassName("form-booking__checkbox");
    var totalPrice = 2700;

    for(var i = 0; i < check.length; i++) {
        if (check[i].checked) {
            totalPrice += parseFloat(check[i].value);
        }
    }
    document.getElementById("price-output").innerHTML = totalPrice;
    document.getElementsById("price-spann").innerHTML = totalPrice;
...
function save_data()
{
	...
	ajax_request.open('POST', '../php/process.php');
